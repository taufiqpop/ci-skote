...
$(() => {
    ...
    $('#table-data').on('click', '.btn-remove', function () {
        ...
        Swal.fire({
            title: 'Anda yakin?',
            html: `Anda akan menghapus submenu "<b>${name}</b>"!`,
            ...
            if (result.isConfirmed) {
                $.post(BASE_URL + 'menu/deleteSubMenu/' + MENU_ID, { id, encrypted_menu_id: MENU_ID, [TOKEN_NAME]: TOKEN_HASH }).done((res) => {
                    console.log(res);
                    if (res.status) {
    ...
    $('#table-data').on('click', '.btn-update', function () {
        ...
        let { id, name, route, parent_id } = data;
    ...
    table = $('#table-data').DataTable({
        ...
        processing: true,
        ajax: {
            url: BASE_URL + 'menu/submenuData/' + MENU_ID,
            type: 'post',
            dataType: 'json',
